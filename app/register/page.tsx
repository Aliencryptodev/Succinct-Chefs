...
import { Twitter, Lock, Loader2, Info } from 'lucide-react'
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await register(twitterUsername, password)
    } catch (error) {
      ...
      <div className="max-w-md w-full">
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold mb-2">Join Succinct Recipes!</h1>
            <p className="text-gray-600 dark:text-gray-300">
              Share your culinary creations with the Twitter community
            </p>
          </div>

          <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl mb-6 flex gap-3">
            <Info className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
            <p className="text-sm text-blue-700 dark:text-blue-300">
              We'll use your Twitter username to display your profile and fetch your avatar
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">Without the @ symbol</p>
            </div>
